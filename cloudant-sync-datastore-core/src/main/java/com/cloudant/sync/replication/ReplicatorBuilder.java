...
public abstract class ReplicatorBuilder<S, T, E> {
    ...
    private URL getIamSessionUrl() {
        // TODO allow over-riding with a system property
        try {
            return new URL("https://iam.bluemix.net/oidc/token");
    ...
    private void setAuthInterceptor(String uriHost, String uriPath, String uriProtocol, int uriPort) {
        if (iamApiKey != null) {
            ...
            IamCookieInterceptor ici = new IamCookieInterceptor(iamApiKey, getIamSessionUrl(), baseURI.toString());
     ...
     * </p>
     * <p>
     *     Note: the replicator will only use IAM to authenticate requests and will not attempt to
